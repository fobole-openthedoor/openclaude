...
仓库：`https://github.com/fobole-openthedoor/openclaude` 分支 **`preserve-reasoning-history`**（不要用 `main`）。Forgejo：`git@forgejo:hacker/openclaude.git`。
...
- 分支必须是 `preserve-reasoning-history`。
...
5. 若 `install-reverse.sh` 不存在（旧 checkout），按同一文件里的 URL 自己下 Ghidra / jadx，clone `https://github.com/zhaoxuya520/reverse-skill.git` 到 `$HOME/tools/reverse-skill`，然后：
...
$HOME/openclaude/scripts/hacker/install-reverse.sh
$EDITOR ~/.config/openclaude/env   # OPENAI_API_KEY
$HOME/openclaude/scripts/hacker/verify-reverse.sh
```
